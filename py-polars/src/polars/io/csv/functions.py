    ...
    if (
        ...
        if columns:
            if not has_header:
                ...
                include_columns = [f"f{int(column[7:])}" for column in columns]
            else:
        ...
        if columns:
            ...
            if projection:
                if columns and len(columns) < len(new_columns):
                    ...
                    raise ValueError(msg)
                # Convert column indices from projection to 'column_0', 'column_1', ...
                # column names.
                current_columns = [f"column_{column_idx}" for column_idx in projection]
            else:
                # Generate autogenerated 'column_0', 'column_1', ... column names for
                # new column names.
                current_columns = [
                    f"column_{column_idx}" for column_idx in range(len(new_columns))
                ]
        else:
